...
**Fronteira de backend (escopo (a), per [ADR-072](../../docs/decisions/ADR-072-role-annotations-plugavel-backend-por-projeto.md)):** apenas `captura_notes` (conteúdo de anotação) resolve via role `annotations`. A signal-queue de coordenação worktree-defer do `captura_backlog` (passo 6) e a baixa-append do gate ADR-069 têm **destino físico local-fixed** em `.claude/local/NOTES.md` (independente do backend declarado) **quando ocorrem** — mecanismo local de concorrência/verify-state, **fora do role**. Sob `null`/`logseq` o gate ADR-069 é no-op por ausência de entry-fonte de anotação, então não há baixa a escrever. Separação física deferida a #157.
