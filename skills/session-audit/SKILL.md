...
- **`captura_backlog` em modo `forge`:** **defer pra `/triage` subsequente** (per passo 1.5). Reportar no done explicitamente sem mutação remota. (O gate de ADR-069 aplica no `/triage` destino — sua corretude para capturas forge NOTES-sourced depende do wiring de `/triage` passo 4 cobrir capturas deferidas, não só a feature-em-curso.)
