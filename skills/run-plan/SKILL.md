...
**Antes do primeiro bloco:** capturar `**Linha do backlog:** <texto>` do `## Contexto` do plano se presente. Se a linha está em `## Próximos` do arquivo do papel `backlog`, cutucar via enum (`AskUserQuestion`, header `Backlog`, opções `Mover para Em andamento` (recomendado) / `Deixar em Próximos`). Mover aplica edit no arquivo do backlog antes do primeiro bloco; edit entra no commit do primeiro bloco. Plano sem o campo, linha não localizada, papel `backlog` "não temos" → skip silente sem mencionar (vale para esta cutucada e para a do passo 4.4). Ver `docs/philosophy.md` → "Ciclo de vida do backlog".
...
4. **Transição final do backlog** — se a referência `**Linha do backlog:**` foi capturada no início do passo 3 E a linha está em `## Em andamento` (ou ainda em `## Próximos`, caso a transição inicial tenha sido recusada), cutucar via enum (`AskUserQuestion`, header `Backlog`, opções `Mover para Concluídos` (recomendado) / `Deixar onde está`). Mover aplica edit como **bloco extra** (atualizar arquivo do papel `backlog` → revisor `code` → micro-commit) antes de seguir para o passo 4.5. Linha não localizada (sumiu desde o registro) → skip silente. Sem referência capturada (plano sem o campo, papel `backlog` "não temos") → skip silente. Ver `docs/philosophy.md` → "Ciclo de vida do backlog".
5. **Backlog harvest** — antes de declarar done, **cutucar** (não bloquear) via enum (`AskUserQuestion`, header `Backlog`) com opções `Nada a registrar` (gate fecha) e `Há itens` (Other → operador lista em prosa: TODO adjacente, tech-debt revelado pela leitura, bug menor avistado de passagem, melhoria não-essencial). A pergunta lembra a regra: só itens **deliberadamente deferidos**, não escopo creep já absorvido pelo plano. Se o operador listar itens, tratá-los como **bloco extra** (atualizar arquivo do papel `backlog` adicionando uma linha por item em `## Próximos` → revisor `code` → micro-commit) antes de declarar done.
6. **Declarar done**.
...
- Não pular o backlog harvest (passo 4.5) — sempre perguntar antes de declarar done. Resposta "nada" é fechamento válido; silenciar é perder itens.
...
- Não silenciar a transição final (passo 4.4) quando a linha está presente e localizada — cutucar é regra; resposta `Deixar onde está` é fechamento válido.
