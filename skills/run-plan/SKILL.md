...
**0. Detecção de modo** (per [ADR-041](../../docs/decisions/ADR-041-campo-modo-runbook-plano-opt-in.md)). Ler `**Modo:**` do `## Contexto` do plano. Ausência = canonical (default total). Presença e valor `runbook` → ativar fluxo alternativo descrito em **## Modo runbook** abaixo (afeta as pré-condições subsequentes, a detecção de warnings pré-loop e os §1/§2/§3 dos Passos). Valor diferente de `runbook` (incluindo `**Modo:** canonical` explícito, `default`, `standard`, etc.) → parar com mensagem `"valor do campo **Modo:** desconhecido: <valor> — valor aceito: runbook (omitir campo para fluxo canonical)"`. Combinação `**Modo:** runbook` + `**Branch:** <nome>` → parar com mensagem `"**Modo:** runbook + **Branch:** incompatíveis — runbook executa no working tree principal, sem worktree"`.
...
Cada warning Backlog/Validação emite `TaskCreate` com marker — mecanismo unificado descrito em §3.5 (per [ADR-039](../../docs/decisions/ADR-039-task-tool-state-keeping-fluxo-longo.md)).
...
Ativado pela pré-condição 0 quando o plano declara `**Modo:** runbook` no `## Contexto` (per [ADR-041](../../docs/decisions/ADR-041-campo-modo-runbook-plano-opt-in.md)). Modo cobre planos de **system-surgery** — operações cuja semântica não é diff git no repo (FS migration, infra reconfig, dotfiles sync, multi-repo edits, ops em `~/`). Bypassa 4 dimensões do canonical:
...
- **Deferida:** durante passos 2 e 3.2, cada captura emergente cria `TaskCreate` com marker `[capture:validacao] <linha>` ou `[capture:backlog] <linha>` no subject (per [ADR-039](../../docs/decisions/ADR-039-task-tool-state-keeping-fluxo-longo.md)). Materialização em §3.5.
...
5. **Captura automática de imprevistos.** Materializar via leitura de `TaskList` filtrada por marker `[capture:*]` (per [ADR-039](../../docs/decisions/ADR-039-task-tool-state-keeping-fluxo-longo.md)). Para cada Task pendente, escrever no destino correto conforme tipo: `[capture:validacao]` → `## Pendências de validação` no plano corrente; `[capture:backlog]` → `## Próximos` do papel `backlog`. Após escrita, marcar Tasks como `completed` via `TaskUpdate`. TaskList sem matches do marker → skip silente (nenhuma captura emergiu na execução).
...
- Não executar fluxo canonical (worktree + micro-commits + reviewer per bloco) em plano com `**Modo:** runbook` — modo runbook reflete que ambiente é o escopo, não o repo (per [ADR-041](../../docs/decisions/ADR-041-campo-modo-runbook-plano-opt-in.md)); aplicar canonical corrompe working tree silenciosamente (worktree dentro do FS migrado, commits artificiais que rollback btrfs sobrescreve, reviewer sobre diff inexistente).
