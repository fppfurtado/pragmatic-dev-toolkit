...
3. **Sanity check de docs user-facing.** Antes do done:
   ...
   - **Skip empírico** se o grep dos identificadores tocados nos paths user-facing da positive list acima retorna vazio. *Identificadores tocados* = filenames base sem extensão de `## Arquivos a alterar` (ex.: `skills/triage/SKILL.md` → `triage`) + nomes de skill/agent/comando textualmente presentes no `## Resumo da mudança` (ex.: `/triage`, `code-reviewer`). Skip silente — evidência empírica de que nenhum doc user-facing referencia o trecho alterado.
   - **Cutucar** caso contrário via `AskUserQuestion` (header `Docs`) com opções `Consistente` / `Listar arquivos a atualizar` (sem `Recommended` — heurística de gatilho aponta gap potencial; ambos os caminhos são igualmente prováveis na prática). `description` da segunda diz "use Other para informar os paths". Antes do enum, listar os referrers concretos do grep como prosa informativa (`<path>:<linha>: <trecho>`) — não candidatos genéricos. Grep é lexical: matches espúrios viram cutucada legítima absorvida pelo `Consistente`. `CHANGELOG` fica fora (responsabilidade do `/release`). Resposta `Consistente` → segue para o passo 3.4. Resposta com listagem (Other) → bloco extra (implementar → `test_command` → revisor `code` → micro-commit) antes do done.
