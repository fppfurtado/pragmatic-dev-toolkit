...
**Critério mecânico de discriminação da 6ª linha** vs. linhas 4-5 (domain/design): linha 6 aplica quando todos os 3 critérios aplicam — (i) paths editados ⊆ path-set narrow (`agents/*.md` ∪ `skills/**/SKILL.md` ∪ `docs/plans/*.md`); (ii) edit cirúrgico (1-N edits localizados sem multi-fase nem decisão estrutural); (iii) substância **não** captura por linhas 4-5 (não é update de `docs/domain.md` nem de `docs/design.md` — paths literais). **Path misto** (path-set narrow ∪ domain/design no mesmo `/triage`): predicado ⊆-strict prevalece → linha 6 **não** aplica; substância cai em linhas 4-5; trigger automático não dispara (operador pode invocar `@prompt-reviewer` manualmente).
