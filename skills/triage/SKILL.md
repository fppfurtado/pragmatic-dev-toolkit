...
- **BACKLOG (papel: `backlog`):** comportamento depende da resolução do papel.
  - **Papel resolvido normalmente:** adicionar **uma linha** em `## Próximos` para a feature em curso. Frase de intenção, sem detalhamento. A transição Próximos → Em andamento é responsabilidade do `/run-plan` no início da execução — registrar Em andamento em `main` produz merge artifact quando o PR é integrado.
