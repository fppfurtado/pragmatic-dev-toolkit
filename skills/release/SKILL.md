...
6. **Gate único via `AskUserQuestion`** (header `Release`) — opções `Aplicar` (executa commit + tag em sequência) e `Editar mensagens` (abre prosa livre — operador descreve quais mensagens revisar e a nova redação; skill aplica os edits, retorna ao item 5 para re-display do bloco e re-pergunta o gate). Cancelamento: operador rejeita o tool call ou responde "abortar" via Other.
7. Após `Aplicar`: `git commit -m "<msg>"` seguido de `git tag -a <tag> -m "Release <tag>"`.
