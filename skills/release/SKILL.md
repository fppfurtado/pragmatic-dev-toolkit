...
Mostrar diff de cada arquivo antes de aplicar via enum (`AskUserQuestion`, header `Diff`, opções `Aplicar` / `Cancelar`). Otimização: quando todos os diffs são mecânicos (mesma troca `X.Y.Z` → `X.Y.Z+1`), agregar numa confirmação única `Aplicar todos` / `Cancelar`.
...
### 4. Aplicar commit + tag (gate único)

Commit e tag são valores 100% derivados das decisões já confirmadas (bump no passo 1, conteúdo do changelog no passo 3) — confirmar cada um separadamente é cerimônia (ver `docs/philosophy.md` → "Convenção de pergunta ao operador"). Skill prepara ambos, mostra num gate único, e executa em sequência mecânica.
...
4. **Verificar colisão de tag.** Se `<tag>` já existe (`git rev-parse <tag> 2>/dev/null`), gap report sem perguntar — skill **não** sobrescreve tag (mover tag é destrutivo de história e fora da blast radius local). Reportar ao operador, sugerir `git tag -d <tag>` se intencional ou bump diferente.
5. **Mostrar `git status` dos arquivos staged** + mensagem de commit + tag a criar, num bloco único. Janela de "abort tardio" — operador interrompe se algo destoar.
6. **Gate único via `AskUserQuestion`** (header `Release`) — opções `Aplicar` (executa commit + tag em sequência) e `Editar mensagens` (Other → operador fornece mensagem de commit e/ou de tag em prosa). Cancelamento: operador rejeita o tool call ou responde "abortar" via Other.
7. Após `Aplicar`: executar `git commit -m "<msg>"` seguido de `git tag -a <tag> -m "Release <tag>"`. Sem perguntas intermediárias — execução mecânica.
...
- Não confirmar separadamente commit e tag — passo 4 tem gate único por construção; perguntar duas vezes por valores derivados da mesma decisão é cerimônia (ver `docs/philosophy.md` → "Convenção de pergunta ao operador").
