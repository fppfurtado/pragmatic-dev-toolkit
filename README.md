...
| `/init-config` | Skill | Interactive wizard for the `<!-- pragmatic-toolkit:config -->` block in `CLAUDE.md`. Asks per role (`decisions_dir`, `backlog`, `plans_dir` canonical/local/null + `ubiquitous_language` canonical/null + `test_command` stack-aware probe) and writes the YAML block. Proactive alternative to the one-shot memoization of the Resolution protocol. |
