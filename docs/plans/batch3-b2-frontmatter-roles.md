...
- ~~Smoke test em uso real (3 cenários: required ausente com creação canonical em `/new-adr`; informational ausente em `/debug`; required ausente sem criação canonical em `/heal-backlog`).~~ **Resolvido por inspeção direta pós-merge.** Verificado em main: (a) 8/8 SKILLs com frontmatter `roles:` declarado; mapping bate com a tabela do plano; (b) CLAUDE.md "Resolution protocol" tem o parágrafo de despacho automático e os 3 trilhos de comportamento default referenciando ADR-003. Smoke comportamental real (validar enum de criação canonical em `/new-adr`, silêncio em `/debug` informational ausente, "informa e para" em `/heal-backlog`) fica para invocação subsequente das skills em uso.
