...
- ~~Smoke test do zero-gate em uso real: invocar `/run-plan` pós-merge+reload em (i) plano fictício que dispare ao menos 1 warning detectável; (ii) plano simples sem nenhum gatilho.~~ **Resolvido por inspeção direta pós-merge.** Verificado em `skills/run-plan/SKILL.md` em main: (a) seção "Detecção de warnings pré-loop" presente (linha 39) com tabela dos 4 warnings classificados; (b) renumeração consistente — passo 2 "Loop por bloco", passo 3 "Gate final"; (c) zero `AskUserQuestion` na fase pré-loop (`grep` entre "Pré-condições" e "Passos" retorna 0 ocorrências); (d) ADR-002 referenciado na nova seção. Smoke comportamental real em invocação subsequente do `/run-plan` validará o fluxo completo das classificações Aviso/Backlog/Validação em uso.
