...
1. **Skip empírico (grep vazio) — alvo do fix.** Plano-fixture que altera apenas `skills/<nome-obscuro>/SKILL.md` (skill não citada em README/install). Identificadores tocados: filename base + nome do skill. Esperado: grep nos paths user-facing retorna vazio → skip silente; agente segue direto para 3.4.
...
- ~~Cenário 1 da `## Verificação manual` (skip empírico via grep vazio, fixture `/triage` step 0) descreve grep com filtros restritivos (`step 0`, `cleanup pós-merge`), não os identificadores literais que a regra implementada usa (filename base `triage` + `/triage` do Resumo). Pela regra literal, a fixture do `/triage` step 0 produz **matches espúrios** em `README.md:9`, `docs/install.md:36/42/45/65` (referências genéricas ao skill) — comportamento real seria cutucada com referrer espúrio absorvida por `Consistente`, não skip silente. Discrepância editorial entre cenário idealizado e regra literal. Reformular: ou (a) usar fixture com identificador genuinamente ausente em docs (skill nova/obscura ainda não documentada); ou (b) reescrever cenário 1 para descrever o caminho real ("cutucada com referrer espúrio → operador descarta via `Consistente`"), tornando-o consistente com as Notas operacionais. Flagado durante `/run-plan run-plan-3-3-skip-empirico` (passo 3.2).~~ **Encerrada 2026-06-07:** opção (a) aplicada — empirical anchor inconsistente removida do Cenário 1 (linha 55) preservando a descrição genérica da fixture; cobertura dos 4 caminhos da regra preservada distintamente; alinhamento editorial com Cenários 2-3 (que também não citam commits específicos).
