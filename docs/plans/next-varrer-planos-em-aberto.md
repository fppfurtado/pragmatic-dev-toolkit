...
- ~~Smoke real dos 7 cenários do `## Verificação manual` pós-`/reload-plugins` em consumer com fixture (`session-audit-skill` com `## Status: Pendente` já materializado em `c75c040` é candidato natural ao Cenário 1). Cobertura textual passou pelo gate `/run-plan §3.2` desta sessão; comportamental fica pendente até reload + invocação real.~~ **Encerrada 2026-06-14:** smoke comportamental dos 7 cenários (sobrepostos com 5 cenários do `next-planos-aberto-enum` + non-canonical Status edge) validado mecanicamente via Lote 4 do `/session-audit` desta sessão CC. 5 fixture plans criados (3 Pendente com mtimes diferenciados, 1 Abortado, 1 Status non-canonical `EmExecucao`); awk classifier `awk '/^## Status$/{flag=1; next} flag && NF{print; exit}' <plans_dir>/<slug>.md` do SKILL §4.6 retornou valores esperados em cada fixture; classifier ignorou silente plans sem `## Status` (cross-check em `archive-plans.md` + `curate-backlog.md` + `wiring-adr-063-caminho-atomico-prompt-reviewer.md` retornou empty); sort `(estado: Pendente > Abortado, mtime: desc, slug: asc)` aplicado em shell pipeline reproduziu ordering esperado. Edge case de non-canonical Status (`EmExecucao`) confirmado skip silente per ADR-060. Fixtures removidos pós-validação. Cobertura textual já havia passado pelo gate `/run-plan §3.2` desta sessão.
