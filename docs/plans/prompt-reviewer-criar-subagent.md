...
- ~~Smoke-test do `prompt-reviewer` em sessão CC nova pós-merge invocando `@prompt-reviewer skills/triage/SKILL.md` — validar findings categorizados nas 4 heurísticas seed (passos conflitantes/vagos/ambíguos/contraditórios) OR close-clean wording; template 4-field; sem sobreposição com escopo de `code-reviewer` ou `doc-reviewer`. Re-rodar 2× se primeira passagem não converge. (Capturado por `/run-plan §3.2` desta execução — agent não descobrível no Task dispatcher da sessão de criação.)~~ **Encerrada 2026-06-14:** smoke real executado em sessão CC fresh via `/next` (operador delegou "execute as validações pendentes que for possível você rodar sozinho"). Retorno produziu 4 findings substantivos (F1 conflito de estado em filtro `forge`; F2 ambiguidade caminho-atômico vs. plano-novo em `docs/plans/*.md`; F3 contradição Mirror linha 188 vs. path-set narrow `docs/plans/*.md`; F5 vago "1-N edits cirúrgicos") + 1 self-retracted (F4 dismissado pelo próprio reviewer, prova de raciocínio real — não simulação). Template 4-field respeitado; categorias nas 4 heurísticas seed corretas; sem sobreposição com code/doc-reviewer. Findings absorvíveis ficaram capturados como linha própria em `BACKLOG.md ## Próximos` (deferidos pra /triage com framing fresh — 5ª iteração inline consecutiva sobre o mesmo SKILL dispararia gatilho de over-correção per memory `feedback_editorial_patterns_emergentes`).
