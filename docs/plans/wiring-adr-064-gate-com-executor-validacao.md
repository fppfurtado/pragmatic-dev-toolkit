...
- ~~§3.4 do SKILL run-plan prescreve "revisor `code` cobre os 2 edits" no bloco extra unificado (BACKLOG mark + remover `## Status`). Mas ambos são editorial puro (1 bullet + 3 linhas removidas), sem código para code-reviewer (YAGNI rubric) operar. Reavaliar: (a) refinar §3.4 para usar `doc-reviewer` (faz sentido editorial); (b) explicitar skip silente quando edits são puramente editorial; ou (c) manter como cinto de segurança (análogo ao bullet anti-blast-radius absorvido em Bloco 2 com mesma justificativa de defesa em profundidade).~~ **Encerrada 2026-06-16:** refator dispatch dinâmico em `/run-plan §3.3` + `§3.4` (caminho-atômico em path-set narrow per ADR-063 — sem ADR novo nem adendo) substitui "revisor `code`" hard-coded por referência ao dispatch path-based de `§2 item 3` (mais-específico-vence per ADR-062). Alternativa (a) realizada mecanicamente: §3.4 paths `.md` (BACKLOG + plan body) → `doc-reviewer` automaticamente; sem skip silente (b) nem cinto de segurança (c).
