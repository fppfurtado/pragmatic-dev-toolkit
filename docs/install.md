...
8a. Em projeto com remote configurado, invocar `/run-plan` num plano simples de 1 bloco → ao concluir, confirmar que o enum `Publicar` aparece com as opções `Push`, `Push + sugerir PR/MR` e `Nenhum`; confirmar que selecionar `Push + sugerir PR/MR` faz `git push` e imprime instrução textual com exemplos por forge (ex.: `gh pr create`, `glab mr create`, ou UI web), sem disparar comando do forge automaticamente; confirmar que em repo sem remote o enum não aparece.
