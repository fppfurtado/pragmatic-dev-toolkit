...
`.worktreeinclude` pode ser tracked (default; equipe compartilha lista) ou gitignored (cada dev tem o seu, útil quando lista varia por dev — paths de fixtures locais, credenciais por máquina, etc.). Plugin permanece agnóstico — `/init-config` cria/atualiza o arquivo independente do estado de tracking quando há sinal de setup: ≥1 role em modo local (per [ADR-018](decisions/ADR-018-replicacao-claude-em-modo-local-init-config.md), garante `.claude/`) ou `CLAUDE.md` gitignored (per [ADR-030](decisions/ADR-030-aceitar-claude-md-gitignored-via-worktreeinclude.md), garante `CLAUDE.md`). **Em modo canonical sem `CLAUDE.md` gitignored, `/init-config` não toca `.worktreeinclude`.** `/note` opera como segundo dispatcher para a invariante `.claude/` em `.worktreeinclude` (per [ADR-018](decisions/ADR-018-replicacao-claude-em-modo-local-init-config.md) Addendum 2026-05-27): probe-and-add idempotente disparado a cada invocação, independente de role local declarada — cobre o caso de `/note` ser usado em projetos sem setup via `/init-config`. `/run-plan` lê e replica o conteúdo na worktree fresca.

Pré-requisitos de runtime dos hooks: `python3` (≥ 3.10) no `PATH`. `run_pytest_python.py` usa sintaxe PEP 604 (`str | None`); versões anteriores falham com `SyntaxError` ao executar o hook. `run_pytest_python` é auto-gated — só dispara em arquivos `.py` que estão sob um diretório com `pyproject.toml`; usa `uv run pytest` quando `uv` está disponível, senão `python -m pytest`. `block_gitignored` é auto-gated em três camadas (file_path vazio, fora de repo git, ou `git` ausente do `PATH` → no-op silencioso); quando dispara, executa `git check-ignore` uma vez e bloqueia (exit 2) se o path estiver coberto pelo `.gitignore` do consumer. Exceção: edits sob `<repo>/.claude/` são allowlisted (território do harness) — load-bearing para o modo local-gitignored do ADR-005. `block_settings_drift` é auto-gated por file path (`endswith("/.claude/settings.json")`) — silente em 99% dos edits; quando dispara, regex match (`/home/<user>/` ou `/Users/<user>/`) no `content`/`new_string` bloqueia (exit 2) com mensagem listando 3 caminhos de remediação (mover para `settings.local.json`, substituir por `$HOME/`/`~/`, editar fora do Claude Code). Per [ADR-040](decisions/ADR-040-block-settings-drift-paths-absolutos-via-hook.md).
