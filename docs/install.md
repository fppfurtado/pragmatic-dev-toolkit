...
2. Após instalar, abrir o Claude Code no workspace e confirmar que `/draft-idea`, `/note`, `/triage`, `/new-adr`, `/run-plan`, `/debug`, `/gen-tests`, `/release`, `/next`, `/init-config`, `/archive-plans`, `/migrate-backlog-to-forge` aparecem em `/help` ou `/plugin list`.
3. Smoke das skills + edição direta de `.env` (verifica `block_env`) + tentativa de edição em path coberto pelo `.gitignore` do consumer (ex.: `.venv/foo.py`, `node_modules/x/index.js` — verifica `block_gitignored`) + tentativa de adicionar permissão com path absoluto (ex.: `"Bash(touch /home/op/test)"`) em `.claude/settings.json` (verifica `block_settings_drift`) + edição de um `.py` num projeto Python (verifica `run_pytest_python`).
3a. Cross-project write via `/note --to` (per [ADR-054](decisions/ADR-054-bridge-cross-project-note-consolidado.md) § Decisão (b)): com `$PROJECTS_DIR` definido e ≥1 projeto irmão inicializado para modo local (`.claude/local/` existente + `.gitignore` cobrindo) (cenário independente do estado do repo corrente), invocar `/note --to <nome-sibling> 'msg test'` → confirmar (a) append em `$PROJECTS_DIR/<nome-sibling>/.claude/local/NOTES.md` com timestamp UTC; (b) relatório cita path absoluto do target + bytes adicionados; (c) `.claude/local/NOTES.md` do repo corrente permanece intocado. Em seguida `/note --to nome-inexistente 'msg'` → confirmar refusal listando até 10 entradas detectadas em `$PROJECTS_DIR/*/`. Por fim, `/note --to /tmp/sem-claude-local 'msg'` (path absoluto sem `.claude/local/`) → confirmar refusal com mensagem `target /tmp/sem-claude-local não inicializado para modo local; abra sessão CC em /tmp/sem-claude-local e rode /note <msg> uma vez para inicializar gates.`. Sem `--to` (default) → fluxo intra-projeto preservado.
...
5b. Invocar `prompt-reviewer` num diff que adiciona, num `.md` de SKILL/agent (`agents/*.md`, `skills/**/SKILL.md`, `docs/plans/*.md`), passo com inconsistência algorítmica conhecida (ex.: instrução sem critério mecânico — `considere X` sem enumeração; pronome com antecedente indeterminado; condição mutuamente exclusiva entre dois gates do mesmo passo) → flag esperado de `passos vagos`, `passos ambíguos`, `passos conflitantes` ou `passos contraditórios` ([ADR-062](decisions/ADR-062-criar-subagent-prompt-reviewer.md) § 4 heurísticas seed). Auto-trigger cobre 2 trajetórias: (i) `/run-plan` per-bloco (ADR-062) e (ii) `/triage` step 5 caminho-atômico em path-set narrow pré-commit ([ADR-063](decisions/ADR-063-caminho-atomico-trigger-prompt-reviewer.md)) — smoke-test cabe em qualquer dos dois pontos.
...
12. Em projeto com `BACKLOG.md` populado em `## Próximos` (N ≥ 1 entries em bullets markdown `- ` ou `* `) e `paths.backlog` em modo file (ausente OR canonical OR local), invocar `/migrate-backlog-to-forge` → confirmar (a) pre-flight `forge-auto-detect` gh-only (em repo glab → para com mensagem orientando replay manual do sub-tool per [ADR-066](decisions/ADR-066-migracao-inicial-cutucada-batched-modo-forge.md) § Limitações); (b) parse via sub-tool retorna N entries; (c) propostas de N títulos via LLM em prosa; (d) cutucada `Migração` única (3 opções: `Aplicar batch completo (Recommended)` / `Revisar antes (loop per-item)` / `Cancelar`) per ADR-066 § Mecânica — 1 confirmação autoriza criação remota das N issues + drain marker + config flip; (e) commit unificado `feat(config): migrar backlog para paths.backlog: forge` + push; (f) comment automático em issue de gap se aberta (cross-ref opt-in). Re-invocação em estado pós-migração (modo forge já flippado) → para com mensagem.
...
Valor `local` = artefato local-gitignored em `.claude/local/<role>/`, aceito por `decisions_dir`, `backlog` e `plans_dir` (recusado por `version_files`/`changelog`). Mecânica (mkdir, probe gitignore, gate `Gitignore`) em [`CLAUDE.md`](../CLAUDE.md) → "Local mode" (ADR-047).

`.worktreeinclude` pode ser tracked (default; equipe compartilha lista) ou gitignored (cada dev tem o seu, útil quando lista varia por dev — paths de fixtures locais, credenciais por máquina, etc.). Plugin permanece agnóstico — `/init-config` cria/atualiza o arquivo independente do estado de tracking quando há sinal de setup: ≥1 role em modo local (per [ADR-047](decisions/ADR-047-modo-local-paths-replicacao-cross-mode.md) § Decisão (b), garante `.claude/`) ou `CLAUDE.md` gitignored (per [ADR-047](decisions/ADR-047-modo-local-paths-replicacao-cross-mode.md) § Decisão (d), garante `CLAUDE.md`). **Em modo canonical sem `CLAUDE.md` gitignored, `/init-config` não toca `.worktreeinclude`.** `/note` opera como segundo dispatcher para a invariante `.claude/` em `.worktreeinclude` (per [ADR-047](decisions/ADR-047-modo-local-paths-replicacao-cross-mode.md) § Decisão (b) — substância Addendum 2026-05-27 absorvida): probe-and-add idempotente disparado a cada invocação, independente de role local declarada — cobre o caso de `/note` ser usado em projetos sem setup via `/init-config`. `/run-plan` lê e replica o conteúdo na worktree fresca.

Pré-requisitos de runtime dos hooks: `python3` (≥ 3.10) no `PATH`. `run_pytest_python.py` usa sintaxe PEP 604 (`str | None`); versões anteriores falham com `SyntaxError` ao executar o hook. `run_pytest_python` é auto-gated — só dispara em arquivos `.py` que estão sob um diretório com `pyproject.toml`; usa `uv run pytest` quando `uv` está disponível, senão `python -m pytest`. `block_gitignored` é auto-gated em três camadas (file_path vazio, fora de repo git, ou `git` ausente do `PATH` → no-op silencioso); quando dispara, executa `git check-ignore` uma vez e bloqueia (exit 2) se o path estiver coberto pelo `.gitignore` do consumer. Exceção: edits sob `<repo>/.claude/` são allowlisted (território do harness) — load-bearing para o modo local-gitignored do ADR-047. `block_settings_drift` é auto-gated por file path (`endswith("/.claude/settings.json")`) — silente em 99% dos edits; quando dispara, regex match (`/home/<user>/` ou `/Users/<user>/`) no `content`/`new_string` bloqueia (exit 2) com mensagem listando 3 caminhos de remediação (mover para `settings.local.json`, substituir por `$HOME/`/`~/`, editar fora do Claude Code). Per [ADR-050](decisions/ADR-050-componentes-plugin-consolidado.md) § Decisão (f).
