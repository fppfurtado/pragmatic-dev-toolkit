...
**Onda 4 fechada (2026-05-12).** Bundle E/F/G_prose + A/B/C/D-NEW shipped via [PR #59](https://github.com/fppfurtado/pragmatic-dev-toolkit/pull/59) (plano `trim-residual-editorial-onda4`). Redução agregada: -122 palavras em 6 arquivos (CLAUDE.md, agents/doc-reviewer.md, templates/plan.md, skills/run-plan, skills/init-config, skills/release). F_arch shipped via commit `066e1ae` (2026-05-12) — caminho-sem-plano + edit cirúrgico direto no main; consolidação do backlog absorvida como sub-fluxo do step 4. H_arch reaberto da seção Diferida e shipped via PR #60 (ver abaixo).

## Diferida

- [x] **H_arch** — [ADR-025](../../decisions/ADR-025-recusar-cross-mode-backlog-local-init-config.md) + [plano](../../plans/recusar-cross-mode-backlog-local-init-config.md) + [PR #60](https://github.com/fppfurtado/pragmatic-dev-toolkit/pull/60) (2026-05-12): recusar cross-mode `backlog: local + plans_dir: canonical` no `/init-config`. Reaberto pós-diferimento — operador absorveu trade-off de YAGNI sem evidência empírica registrada (gatilho original do diferimento). ADR-025 sucessor parcial de ADR-005 § Limitações; recusa assimétrica ("direção do leak" é o critério — texto privado→público é incoerente, leitor privado consumindo público é normal). Defensividade assimétrica em `/triage` step 1 (opção (b) sobre 3 alternativas pós-finding alto do design-reviewer); `/run-plan` sem check. 11 findings absorvidos pré-commit (6 no ADR + 5 no plano). Saldo líquido ~7 linhas (~10 removidas, ~3 adicionadas). Validação manual pendente para smoke-test em consumer real pós-release.

**Roadmap 2026-05-12 totalmente cumprido (2026-05-12).** 4 ondas + item Diferido fechados: Onda 1 (3 ADRs C/B/E_arch via PRs #54-#55), Onda 2 (A_arch/A_prose/C_prose via PR #56), Onda 3a (G/D_arch via PR #57), Onda 3b (B/D_prose via PR #58), Onda 4 (E/F/G_prose + A/B/C/D-NEW via PR #59; F_arch via commit `066e1ae`; H_arch via PR #60). Total: 6 ADRs novos (020-025), 5 planos shippados, 5 PRs + 1 commit cirúrgico direto.
