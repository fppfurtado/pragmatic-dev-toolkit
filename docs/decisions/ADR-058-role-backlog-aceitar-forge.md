...
Aplica a 4 superfícies primárias: `/triage` step 4 (criar issue), `/next` passo 3 (fechar issue de feature implementada via evidência forte), `/run-plan §3.4` (fechar issue da feature em done), `/curate-backlog` H1 (fechar issue com gatilho temporal vencido). **Sub-caso editorial — 1ª instância (forma granular):** Consolidação do `/triage` step 4 (releitura para detectar duplicatas/obsolescência pós-edit) — edits descritos pelo operador via `Other` que impliquem mutação remota (fechar/editar issue já existente) seguem a mesma policy de cutucada por mutação granular. Não é superfície primária porque a mutação não é a operação canonical do step 4 (que é criar); mas a categoria editorial é idêntica (blast radius remoto) e a policy é a mesma.

**Sub-caso editorial — 2ª instância (forma batched-com-seleção):** `/run-plan §3.5` captura `[capture:backlog]` em modo forge — Task pendente materializa como bullet em `## Capturas backlog em modo forge` do plano corrente **sem cutucada individual**; cutucada **batched-com-seleção** (`Aplicar todas` / `Selecionar quais` / `Manter como pendentes`) ao final de §3.5. Categoria editorial idêntica à 1ª instância (blast radius remoto, policy de cutucada-por-mutação preservada — cada decisão `Aplicar`/`Manter`/`Selecionar` é confirmação explícita de mutação remota); difere apenas na **forma de apresentação**. Justificativa da forma distinta: signal `[capture:backlog]` acumula em múltiplos passos do `/run-plan` (warnings pré-loop + passo 2 + passo 3.2 — até 5+ markers); cutucada granular geraria N cliques fadigosos. Variação batched-com-seleção mantém controle granular (operador escolhe subset via `Selecionar quais`) sem N cutucadas. Mecanismo paralelo a `## Pendências de validação` que também é gate único editorial ao final do plano. Também resolve potencial tensão com ADR-002 § Decisão "skill nunca interrompe por cutucada na fase pré-loop": warnings pré-loop classificados como `[capture:backlog]` em modo forge não disparam cutucada na materialização individual — cutucada bate no done (gate único editorial paralelo a §3.4), não na fase pré-loop nem na materialização individual.
...
| `/run-plan §3.5` modo forge | Materializar `[capture:backlog]` | Escrever bullet em `## Capturas backlog em modo forge` do plano corrente (sem cutucada individual). Cutucada batched-com-seleção ao final de §3.5 (`Aplicar todas` / `Selecionar quais` / `Manter como pendentes`). Bullets aplicados viram refs `- #<número>: <linha>` no plano. 2ª instância do sub-caso editorial de § (e) — forma batched (vs 1ª instância granular da consolidação `/triage step 4`). |
