...
3. **Linha está em `## Concluídos`, não em `## Próximos`.** Linha em Próximos → **silente** (item não terminou; sem ruído no relatório).
   ...
   > **Cross-ref [ADR-060](ADR-060-heuristica-completude-planos-via-status.md) (2026-06-12):** ADR-060 é sucessor parcial deste critério — adiciona campo `## Status` no body do plano carregando `Pendente`/`Abortado` (estados que git/forge não captura e que critério 3 colapsa em "silente"). Critério 3 permanece signal canonical de `Concluído` via cadeia editorial; ADR-060 complementa pros 2 estados novos sem substituir esta cadeia. `/archive-plans` usa ADR-022 cadeia como primária quando field ausente; consumir field via ADR-060 § Modelo de signal é responsabilidade de skills downstream (`/next varrer planos em aberto`).
