...
- **Store em `.claude/local/NOTES.md` modo local-gitignored** (Q1=B). Path fixo doutrinário, non-role. Por-projeto, não-versionado. Atendido critério #2 do operador via referência conversacional (não-automático). Estendido por [ADR-018](ADR-018-replicacao-claude-em-modo-local-init-config.md) Addendum (2026-05-27): `/note` passo 1 aplica silentemente a invariante `.claude/` em `.worktreeinclude` por probe-and-add idempotente, garantindo que worktrees do `/run-plan` enxerguem `NOTES.md` mesmo quando consumer não declarou role local (segundo dispatcher universal, ortogonal ao trigger condicional do `/init-config` step 4.5).
