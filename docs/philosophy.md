...
Skills perguntam em dois modos complementares — **enum** (escolha discreta) e **prosa livre** — e a escolha entre eles não é estética: errar o modo gera ou cerimônia (enum em pergunta livre) ou improviso (prosa em escolha discreta). Use enum quando a resposta esperada é um nome/escolha concreto; use prosa quando a resposta exige explicação, exemplo, justificativa, ou descrição naturalmente aberta. Quando a maioria das respostas reais cairia em "Other" do enum, o modo certo era prosa desde o início.
